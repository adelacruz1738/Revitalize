...
// Array of songs for music player
const songs = [
  {
    id: 1,
    title: "Irish Coast",
...
// Maps songs to corresponding button id
const SongList = () => {
  const list = songs.map((song) => {
    return (
      <Link to={`/Meditate/${song.id}`}>
        <button className="type_btn">
          <h3 className="btn_text">{song.title}</h3>
        </button>
      </Link>
    );
  });
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <div className="mediT_header">
          <h1>Please choose a meditation technique</h1>
        </div>
        <SongList />
...
export default connect(mapStateToProps, { logout })(Meditationtype);
